...
class PoseDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        metadata_file_path = os.path.join(self.data_dir, "../metadata_normalized.csv")
        self.metadata_frame = pd.read_csv(metadata_file_path)
        
        valid_indices = []
        for idx in tqdm(range(len(self.metadata_frame)), desc="Validating Images"):
            img_full_path = os.path.join(self.data_dir, os.path.basename(self.metadata_frame.iloc[idx]['path']))
